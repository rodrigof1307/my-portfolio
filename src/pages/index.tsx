  ...
  useEffect(() => {
    ...
    const handleProjects = () => {
      if (scrollY >= sections[2].offsetTop - 500) {
        const ratio =
          (scrollY - sections[2].offsetTop) /
          (sections[3].offsetTop - sections[2].offsetTop);
        if (0.05 <= ratio && ratio < 0.3) {
          setHighlightedEmail(emailInfo[0]);
          if (nftRenter[0]) {
            nftRenter[0].scrollTo({
              top:
                ((nftRenter[0].scrollHeight - nftRenter[0].clientHeight) /
                  0.25) *
                (ratio - 0.05),
              behavior: "smooth",
            });
        ...
        if (0.4 <= ratio && ratio < 0.65) {
          setHighlightedEmail(emailInfo[1]);
          if (bugPointer[0]) {
            bugPointer[0].scrollTo({
              top:
                ((bugPointer[0].scrollHeight - bugPointer[0].clientHeight) /
                  0.25) *
                (ratio - 0.4),
              behavior: "smooth",
            });
    ...
    const handleEducation = () => {
      if (scrollY >= sections[3].offsetTop - 500) {
        const ratio =
          (scrollY - sections[3].offsetTop) /
          (sections[4].offsetTop - sections[3].offsetTop);
        ...
        if (0.15 < ratio && ratio < 0.3) {
          setEducationCode([
            helperMasters.substring(
              0,
              Math.ceil(((ratio - 0.15) / 0.15) * helperMasters.length)
            ),
            "",
            "",
          ]);
          ...
          setEducationCode([
            helperMasters,
            helperBachelors.substring(
              0,
              Math.ceil(((ratio - 0.325) / 0.15) * helperBachelors.length)
            ),
            "",
          ]);
          ...
          setEducationCode([
            helperMasters,
            helperBachelors,
            helperCourses.substring(
              0,
              Math.ceil(((ratio - 0.5) / 0.15) * helperCourses.length)
            ),
          ]);
        }
      }
      ...
      <main className="-mt-20 sm:-mt-10 md:-mt-6 lg:mt-0">
        ...
        <div className="initial-animation">
          <SectionLayout title="About Me">
            <p className="text-md m-auto mx-auto mb-9 font-mono xxs:w-[33rem] xs:w-[40rem] md:w-[80rem] md:text-xl 2xl:w-[92rem]">
              Hi! My name is <b>Rodrigo Fernandes</b> and I’m a <b>Frontend</b>{" "}
              and <b>Mobile Software Engineer</b> based in{" "}
              <b>Lisbon, Portugal</b>.
            </p>
            <Skills />
          </SectionLayout>
          ...
          <SectionLayout title="Projects" isStickySection>
            <Projects
              ...
              setHighlightedEmail={setHighlightedEmail}
              removeSticky={removeProjectsSticky}
            />
          </SectionLayout>
          <SectionLayout title="Education" isStickySection>
            <Education
              educationCode={educationCode}
              completeEducationCode={completeEducationCode}
              removeSticky={removeEducationSticky}
            />
          </SectionLayout>
